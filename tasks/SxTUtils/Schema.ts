...
class SchemaManager {
    ...
    async backupSchema(taskArgs: TaskArgs): Promise<Result> {
        ...
        const archiveFormat = taskArgs.format || 'zip';
        if (!Utils.isFormat(archiveFormat)) {
          throw {
            success: false,
            message: `Invalid archive format: ${archiveFormat}. Allowed formats are 'zip' and 'tar'.`,
          };
        }
      
        const output = fs.createWriteStream(path.join(process.cwd(), `${taskArgs.schema}.${archiveFormat}`));
    ...
    async manageSchemaDDL(hre: HardhatRuntimeEnvironment, taskArgs: TaskArgs, action: 'create' | 'drop'): Promise<SxTResult> {
      ...
      const sql: RenderSQLResult = await this.createSchema(taskArgs, Utils.getSchemaDir(taskArgs.schema));
      let result: SxTResult;
      if (sql.success) {
        if (sql.sql) {
          console.log('----------------------------------------')
          console.log(sql.sql)
          console.log('Creating Schema:', taskArgs.schema.toUpperCase())
          let [ddlSuccess, ddlError] = await hre.sxtSDK.DDL(sql.sql, [], "");
          //console.log('DDL Success: ', ddlSuccess)
          //console.log('DDL Error: ', ddlError)
          if (ddlError) {
            result = { 
              success: false,
